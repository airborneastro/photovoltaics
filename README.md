...
node red flow adapted from https://github.com/mkgeiger/node-red-sma
 The node red flow extracts information from the web interface of an SMA Tripower Smart Energy hybrid inverter in combination with a SMA Sunny Home Manager (SHM). Essentially,the flow makes three http requests in sequence: 
...
With the sid, ask for all available data with:
    ...
    curl -X POST https://sma3015xxxxxx/dyn/getAllOnlValues.json?sid=A53TnUC-0MQ-dWCA -k -H 'Content-Type: application/json' -d '{"destDev": []}'
    
The result is a (long) json string with the data as key: value pairs that can be extracted by parsing the json string. The file "valueID_STP10_SE.xlsx" contains all keys with their parameter descriptions that I found out by analyzing the web interface with the developer tools of the Firefox browser. The third step is to logout from the web interface with:

    curl -X POST https://sma<serialnumber>/dyn/logout.json?sid=A53TnUC-0MQ-dWCA -k -H 'Content-Type:application/json' -d '{}'

The red node flow then sends the requested data into an influxdb2 database. 
...
The organisation of the database with its buckets, measurements and fields as needed for the following grafana dashboard is described in "influxdb2_data_organisation"
...
The queries for the grafana panels are written in the "flux" language for influxdb2
